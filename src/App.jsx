...
const App = () => {
  ...
  const [animatingChecks, setAnimatingChecks] = useState({});
  ...
  const toggleCheck = (id) => {
    ...
    const anims = { ...animatingChecks };
    updated.forEach(item => {
      const old = currentTripData.find(i => i.id === item.id);
      if (old) {
        if (old.done !== item.done) anims[`loc-${item.id}`] = true;
        if (old.transportDone !== item.transportDone) anims[`trans-${item.id}`] = true;
      }
    });
    setAnimatingChecks(anims);
    setTimeout(() => {
      setAnimatingChecks(prev => {
        const next = { ...prev };
        Object.keys(anims).forEach(k => delete next[k]);
        return next;
      });
    }, 150);
  ...
  const toggleTransportCheck = (id) => {
    ...
    updated.forEach(item => {
      const old = currentTripData.find(i => i.id === item.id);
      if (old && old.transportDone !== item.transportDone) {
        anims[`trans-${item.id}`] = true;
      }
    });
    setAnimatingChecks(anims);
    setTimeout(() => {
                        ...
                        <div key={item.id} id={`card-${item.id}`} className="relative mb-0">
                          ...
                          <div className={`relative flex ${isMobileView ? 'gap-2' : 'gap-4'} group z-10 pt-2`}>
                            <div className="flex flex-col items-center w-14 shrink-0 bg-transparent">
                              <button onClick={() => toggleCheck(item.id)} className={`z-10 w-9 h-9 rounded-full border-4 flex items-center justify-center font-black text-xs transition-all shadow-lg ${animatingChecks[`loc-${item.id}`] ? 'scale-90' : 'hover:scale-110 scale-100'} ${item.done ? 'bg-gray-500 border-gray-500/20 text-white' : (isDarkMode ? 'bg-[#0f1115] text-blue-500 border-blue-500' : 'bg-[#fdfbf7] text-blue-600 border-blue-500')}`}>
                          ...
                          {idx < group.items.length - 1 && (
                            <div key={`transport-${item.id}`} id={`transport-${item.id}`} className={`flex ${isMobileView ? 'gap-2' : 'gap-4'} py-3 items-center relative z-10 group`}>
                              <div className="w-14 shrink-0 bg-transparent flex flex-col items-center justify-center relative z-20 -translate-y-5">
                                <button onClick={() => toggleTransportCheck(item.id)} className={`w-6 h-6 rounded-full z-20 border-[3px] flex items-center justify-center transition-all shadow-lg ${animatingChecks[`trans-${item.id}`] ? 'scale-90' : 'hover:scale-110 scale-100'} ${item.transportDone ? 'bg-gray-500 border-gray-500/20 text-white' : (isDarkMode ? 'bg-[#0f1115] text-yellow-500 border-yellow-500' : 'bg-[#fdfbf7] text-yellow-600 border-yellow-500')}`}>
                              ...
                              <div className={`flex-1 flex items-center justify-between px-3 py-2 rounded-xl border border-dashed shadow-sm transition-all transform-gpu ${isDarkMode ? 'bg-white/[0.03] border-white/5' : 'bg-white/60 border-gray-200'} ${item.transportDone ? 'opacity-50' : ''}`}>
